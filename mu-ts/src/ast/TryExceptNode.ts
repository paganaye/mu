...
import { MuError } from '../parser/Utils';
...
export class TryExceptNode extends AstNode {
    ...
    eval(runtimeContext: RuntimeContext): any {
        ...
        try {
            ...
            const name = (err instanceof MuError) ? err.name : typeof (err);
            const message = (err as any)?.message || String(err);
            const moduleName = (err instanceof MuError) ? err.module : 0;
            const line = (err instanceof MuError) ? err.line : 0;
            const column = (err instanceof MuError) ? err.column : 0;
